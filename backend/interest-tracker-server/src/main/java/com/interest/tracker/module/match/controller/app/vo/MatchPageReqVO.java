package com.interest.tracker.module.match.controller.app.vo;

import com.interest.tracker.framework.common.pojo.PageParam;
import io.swagger.v3.oas.annotations.media.Schema;
import lombok.Data;
import lombok.EqualsAndHashCode;

import java.time.LocalDate;

/**
 * 比赛分页查询请求 VO
 *
 * @author interest-tracker
 */
@Schema(description = "比赛分页查询请求 VO")
@Data
@EqualsAndHashCode(callSuper = true)
public class MatchPageReqVO extends PageParam {

    @Schema(description = "用户ID（系统自动填充）", hidden = true)
    private Long userId;

    /**
     * 比赛类型：1-联赛 2-杯赛 3-友谊赛
     * 枚举 {@link com.interest.tracker.module.match.enums.MatchTypeEnum}
     */
    @Schema(description = "比赛类型：1-联赛 2-杯赛 3-友谊赛", example = "1")
    private Integer matchType;

    /**
     * 观赛方式：1-现场 2-直播 3-回放
     * 枚举 {@link com.interest.tracker.module.match.enums.WatchTypeEnum}
     */
    @Schema(description = "观赛方式：1-现场 2-直播 3-回放", example = "2")
    private Integer watchType;

    @Schema(description = "主队名称", example = "巴塞罗那")
    private String homeTeamName;

    @Schema(description = "客队名称", example = "皇家马德里")
    private String awayTeamName;

    @Schema(description = "比赛场地", example = "老特拉福德")
    private String venue;

    @Schema(description = "开始比赛日期", example = "2025-01-01")
    private LocalDate startMatchDate;

    @Schema(description = "结束比赛日期", example = "2025-12-31")
    private LocalDate endMatchDate;

}

