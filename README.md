...
一个整合摄影、影视、音乐、图书、旅游、演唱会、球赛等多兴趣领域的个人内容管理与数据分析平台
...
**Interest Tracker** 是一个现代化的个人兴趣内容管理与分析平台，帮助用户统一管理摄影、影视、音乐、图书、旅游、演唱会、球赛等多兴趣领域的记录，通过数据可视化分析用户的兴趣习惯，并提供智能推荐功能。

### 核心价值

- 🎯 **统一管理**：多兴趣领域的统一管理平台，告别数据分散
...
#### 📸 摄影模块
- 照片上传与管理（支持单张/批量上传）
- EXIF 数据自动提取（拍摄时间、设备、ISO、光圈、快门等）
- 照片预览与展示（瀑布流布局、时间线视图、地图视图）
- 相册分类管理
- 照片标签与分类
- 拍摄数据分析（拍摄频率、常用设备、参数偏好、地点热力图）
...
│   │           ├── music/                  # 音乐模块
│   │           └── user/                   # 用户模块
│   │           # 注：图书、旅游、演唱会、球赛等模块的数据库表已设计，后端接口待开发
│   │
│   ├── database/                      # 数据库脚本
│   │   └── init.sql                   # 初始化 SQL
│   │
...
- **图书表**：`book`、`book_record` - 图书信息和阅读记录
- **旅游表**：`travel_place`、`travel_record` - 旅游地点和旅游记录
- **演唱会表**：`concert`、`concert_record` - 演唱会信息和观看记录
- **照片表**：`photo` - 照片信息（支持关联旅游、演唱会等）
- **标签表**：`tag` - 标签管理
