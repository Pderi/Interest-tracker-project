  ...
  <div class="movie-page">
    ...
    <div class="min-h-[120px] relative">
      ...
      <EmptyState
        ...
        title="暂无记录"
        description="点击右上角「添加记录」开始你的观影之旅"
      >
        <template #icon>
      ...
      >
        <template
          ...
          :key="movie.recordId"
        >
          <div class="flex flex-col gap-2 movie-card-wrapper" :style="{ animationDelay: `${index * 50}ms` }">
            <!-- 影视卡片 -->
            <AnimatedCard
              variant="3d"
            ...
            >
              <div class="rounded-2xl overflow-hidden">
                <!-- 封面 -->
                <div class="relative aspect-[2/3] overflow-hidden movie-poster-container">
                  <div class="poster-overlay"></div>
                  <img
                    v-if="movie.posterUrl && !imageErrorMap[movie.movieId]"
                    :src="movie.posterUrl"
                    :alt="movie.title"
                    class="movie-poster w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                    loading="lazy"
                  ...
                  <div class="absolute top-3 right-3 status-badge-wrapper">
                    <div class="status-badge" :class="`status-badge-${movie.watchStatus}`">
                      ...
                      <el-tag
                        ...
                        @click.stop="handleStatusClick(movie)"
                        @mouseenter="handleStatusHover"
                      >
                        <span class="status-text">{{ getStatusLabel(movie.watchStatus) }}</span>
                      </el-tag>
                    </div>
                  </div>
                  ...
                  >
                    ...
                    <span class="text-white text-sm font-semibold rating-value">{{ movie.personalRating }}</span>
                  </div>
                </div>
                ...
                <div class="p-4 flex flex-col gap-2 glass-effect">
                  ...
                  >
                    <AnimatedTag
                      ...
                      class="movie-tag-item"
                    >
                      {{ tag }}
                    </AnimatedTag>
                  </div>

                  <div class="mt-2 flex justify-between items-center movie-actions">
                    <AnimatedButton
                      variant="outline"
                      size="small"
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <p class="text-sm text-gray-300 leading-relaxed">
                      <span class="text-[#00d4ff]/60">"</span>{{ movie.comment }}<span class="text-[#00d4ff]/60">"</span>
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div class="guide-modal glass-effect">
          <div class="guide-modal-header">
            <div>
              ...
              <h3 class="guide-modal-title">3 步搞定你的观影清单</h3>
            </div>
...
const GUIDE_STORAGE_KEY = 'movie_onboarding_seen'
const guideVisible = ref(false)
const guideAutoShown = ref(false)
const guideSteps = [
  {
    title: '添加第一条记录',
    description: '点击右上角“添加记录”，录入想看的电影或剧集，支持上传封面与标签。',
    action: '添加记录',
  },
  {
    title: '用筛选快速定位',
    description: '状态筛选 + 关键词搜索，快速找到在看/已看，列表支持分页。',
    action: '筛选/搜索',
  },
  {
    title: '评分与评价',
    description: '在卡片上直接编辑、评分、标记状态，保持你的观影清单实时更新。',
    action: '编辑/评分',
  },
]
...
async function loadMovies() {
  try {
    ...
    movieList.value = res.data.page?.list || []
...
function handleStatusClick(movie: MoviePageItem) {
  // 点击状态标签时，可以快速筛选该状态
  changeStatus(movie.watchStatus)
  // 滚动到顶部
  window.scrollTo({ top: 0, behavior: 'smooth' })
  // 显示提示
  ElMessage.success(`已筛选：${getStatusLabel(movie.watchStatus)}`)
}
...
/* 不同类型状态的额外效果 */
.status-badge-1 .status-tag {
  border: 1px solid rgba(64, 158, 255, 0.3);
}

.status-badge-1:hover .status-tag {
...
.status-badge-2 .status-tag {
  border: 1px solid rgba(230, 162, 60, 0.3);
}

.status-badge-2:hover .status-tag {
...
.status-badge-3 .status-tag {
  border: 1px solid rgba(103, 194, 58, 0.3);
}

.status-badge-3:hover .status-tag {
  border-color: rgba(103, 194, 58, 0.6);
  box-shadow: 
    0 4px 12px rgba(103, 194, 58, 0.4),
    0 0 20px rgba(103, 194, 58, 0.2);
  animation: status-success-glow 2s ease-in-out infinite;
}
...
@keyframes status-success-glow {
  ...
  50% {
    box-shadow: 
      0 4px 12px rgba(103, 194, 58, 0.6),
      0 0 30px rgba(103, 194, 58, 0.4);
  }
}

.status-badge-4 .status-tag {
  border: 1px solid rgba(245, 108, 108, 0.3);
}

.status-badge-4:hover .status-tag {
  border-color: rgba(245, 108, 108, 0.6);
  box-shadow: 
    0 4px 12px rgba(245, 108, 108, 0.4),
    0 0 20px rgba(245, 108, 108, 0.2);
}
...
.movie-tag-item:nth-child(1) { animation-delay: 0.1s; }
.movie-tag-item:nth-child(2) { animation-delay: 0.2s; }
.movie-tag-item:nth-child(3) { animation-delay: 0.3s; }
.movie-tag-item:nth-child(4) { animation-delay: 0.4s; }
